...
import { setSelectedGridView } from "../../features/selectedGridViewSlice";
...
// const ClearData = Symbol("clearData");
...
export default function GridHeader() {
  ...
  const selectedGridView = useSelector((state) => state.selectedView);
  ...
  // save template form local state
  ...
  const transpositionGridRef = React.createRef();
  ...
  async function saveGridData() {
    var api = mainGridRef.current.api;
    ...
    changedRows.forEach((rec) => {
      for (const [key] of Object.entries(rec)) {
    ...
    try {
      ...
      let hasErrors = false;
      result.forEach((r) => {
        const node = mainGridRef.current.api.getRowNode(r.id);
        const rec = node.data;
  ...
  */
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      ...
      console.log(`Running get org objects useEffect`);
    ...
    loadInitialData();
  }, [objectOptions, dispatch, userInfo, enqueueSnackbar]);
    ...
    const objChanged = async () => {
      ...
      try {
        ...
        if (templateData.length > 0 && defaultTemplates.length === 0) {
          ...
          // use this template
          dispatch(setSelectedTemplate(tmpOptions[0]));
        }

        const defaultQueryResult = await ghf.getDefaultQueries(
          selectedObject.id,
          userInfo.orgid,
          true, // default
          true, // is_active
        ...
        if (defaultQueries.length === 0) {
          // pick the first one
          dispatch(setSelectedQuery(qryOptions[0]));
        }
        ...
        if (relPreferences.length === 1) {
          ...
          prefs.forEach((p) => {
            // get metadata if needed
            ghf
              .getObjectMetadata(p.obj, userInfo, objectMetadata)
              .then((res) => {
                if (res.status === "error") {
                  throw new Error(
        ...
        if (objPrefResult.records.length > 0) {
          ...
          dispatch(setSelectedGridView(optionsList[0]));
        }
      ...
      >
        ...
        >
          ...
          <Autocomplete
            ...
            onChange={(event, newValue) => {
              dispatch(setSelectedGridView(newValue));
            }}
            sx={{ ml: 5, width: 150 }}
          />
        </Toolbar>
